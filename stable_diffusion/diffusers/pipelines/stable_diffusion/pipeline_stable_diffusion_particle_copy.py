                ...
                if do_classifier_free_guidance:
                    ...
                    if dino is not None:
                        ...
                        if sigma > sigma_break:
                            ...
                            coeff_ = coeff
                            grad_phi = 2 * weights * diff / h_t * 2 * coeff_
                ...
                if i + 1 in restart_list.keys():
                    ...
                    for restart_iter in range(restart_list[restart_idx][1]):
                        ...
                        for j, new_t in enumerate(new_t_steps[:-1]):
                            ...
                            sigma = all_sigmas[new_t.cpu().numpy()]
                            sigma = sigma.to(latents.device)
                            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                            # perform guidance
                            if do_classifier_free_guidance:
                                ...
                                if dino is not None:
                                    ...
                                    alpha_prod_t = self.scheduler.alphas_cumprod[new_t.cpu().numpy()]
                                    alpha_prod_t.requires_grad_(True)
                                    beta_prod_t = 1 - alpha_prod_t
                                    ...
                                    if sigma > 5:
                                        ...
                                        distance = torch.norm(diff, p=2, dim=-1, keepdim=True)
                                        num_images = latents_vec.shape[0]
                                        h_t = (distance.mean(dim=1, keepdim=True) * num_images / (
                                                    num_images - 1)) ** 2 / np.log(
                                            num_images)
                                        weights = torch.exp(- (distance ** power / h_t))
